...
//    }
    ...
    // if (!angular.equals(newValue, oldValue)) {
    // if ($rootScope.currencies && $scope.selectedInventory != null &&
    // $rootScope.currencies[$scope.selectedInventory.FUNDS]) {
    // $scope.fundsSymbol =
    // $rootScope.currencies[$scope.selectedInventory.FUNDS].symbol;
    // }
    ...
    // {
    // // if
    // ($rootScope.exchange[$scope.selectedInventory.FUNDS].hasOwnProperty(key))
    // {
    // // //
    // // if (new Date(key).getTime() >= new
    // Date($scope.selectedInventory.DATE).getTime()) {
    // // exchangerate =
    // $rootScope.exchange[$scope.selectedInventory.FUNDS][key];
    // // break;
    // // }
    // //
    // // }
    ...
    // // $scope.selectedInventory.PST = 'N';
    // // }
    // // if ($scope.lastOrderFunds != $scope.selectedInventory.FUNDS &&
    // $scope.selectedInventory.FUNDS != 'C' && $scope.selectedInventory.FUNDS
    // != 'U') { // foreign
    // // $scope.selectedInventory.SANDH = 'N'; // manual
    // // } else if ($scope.lastOrderFunds != $scope.selectedInventory.FUNDS &&
    // ($scope.selectedInventory.FUNDS == 'C' || $scope.selectedInventory.FUNDS
    // == 'U')) { // american/canadian
    ...
    // //
    // // angular.forEach($scope.selectedInventory.dporderdetails,
    // function(detail) {
    // //
    // // var itemp = parseInt(detail.LQTY) * parseFloat(detail.LPRICE) * (1 -
    // (parseInt(detail.LDISC) / 100));
    // //
    ...
    // // // to have NY tax
    // // $scope.selectedInventory.NYTAX = ($scope.selectedInventory.COUNTY ==
    // null || $scope.selectedInventory.FUNDS != 'U' ? 0 :
    // $rootScope.county_rates[$scope.selectedInventory.COUNTY]);
    // // $scope.selectedInventory.NYTCALC = ($scope.selectedInventory.ECONV +
    // $scope.selectedInventory.ESHIP) * ($scope.selectedInventory.NYTAX / 100);
    // //
    // // $scope.selectedInventory.GTOTAL = $scope.selectedInventory.ECONV +
    // $scope.selectedInventory.ESHIP + $scope.selectedInventory.HSTCALC +
    // $scope.selectedInventory.GSTCALC + $scope.selectedInventory.PSTCALC +
    // $scope.selectedInventory.NYTCALC;
    //
    // }
    // }
    //
...
//			    // $scope.selectedInventory.is_modified =
//			    // $contact.is_modified = false;
...
//			    $scope.inventoryDatatable.dataTable.api().draw(false);  // TODO - needs data actually updated..
...
.controller('InventorySearch', function($scope, $rootScope, $modal, $sails, $timeout) { // $contact
	...
	// });
	// $rootScope.county_rates[data.county_codes[i].CODE] =
	// data.county_codes[i].MCAT_LO;
	// }
	...
	// id : data.currencies[i].id,
	// selector_label : data.currencies[i].name + ' to United States
	// Dollar',
	// label : data.currencies[i].name
	// });
	//
...
//	$scope.inventoryDatatable.dataTable.fnGetData()
	...
	$scope.$on('event:dataTableLoaded', function(event, data) {
		...
		for ( var sfield in $scope.inventory_search) {
			...
			if (sfield == 'quantity_MIN' && $scope.inventory_search.quantity_MIN !== '' && parseInt(data[$scope.column_index['quantity']]) < parseInt($scope.inventory_search.quantity_MIN)) {
			    return false;
			} else if (sfield == 'quantity_MAX' && $scope.inventory_search.quantity_MAX !== '' && parseInt(data[$scope.column_index['quantity']]) < parseInt($scope.inventory_search.quantity_MAX)) {
			    return false;
			}
		    } else if (sfield == 'date_added_MIN' || sfield == 'date_added_MAX') {
			if (sfield == 'date_added_MIN' && $scope.inventory_search.date_added_MIN !== '' && $scope.inventory_search.date_added_MIN !== null && (data[$scope.column_index['date_added']]) < ($scope.inventory_search.date_added_MIN)) {
			    return false;
			} else if (sfield == 'date_added_MAX' && $scope.inventory_search.date_added_MAX !== '' && $scope.inventory_search.date_added_MAX !== null && (data[$scope.column_index['date_added']]) > ($scope.inventory_search.date_added_MAX)) {
			    return false;
			}
		    } else if (sfield == 'price_MIN' || sfield == 'price_MAX') {
			if (sfield == 'price_MIN' && $scope.inventory_search.price_MIN !== '' && parseInt(data[$scope.column_index['price']]) < parseInt($scope.inventory_search.price_MIN)) {
			    return false;
			} else if (sfield == 'price_MAX' && $scope.inventory_search.price_MAX !== '' && parseInt(data[$scope.column_index['price']]) > parseInt($scope.inventory_search.price_MAX)) {
			    return false;
			}
			...
			return false;
		    } else if (typeof ($scope.inventory_search[sfield]) == 'string' && $scope.inventory_search[sfield] != '' && data[$scope.column_index[sfield]].indexOf($scope.inventory_search[sfield]) == -1) {
			return false;
		    }
		}
		return true;
	...
	function rowClicked(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
		...
		$rootScope.selectedLotChanged = true;
		$rootScope.changes_pending = false;
		$scope.inventoryDatatable.dataTable.api().draw(false);  // redrawing it makes it see the matched selectedLot id and draw it green.
	    }, 0);
