...
var app = angular.module('xenon-app', [ 'ngCookies', 'ui.router', 'ui.bootstrap',  'oc.lazyLoad', 'ui.sortable',  'preloaded', 'xenon.controllers', 'xenon.controllers.inventory', 'xenon.controllers.history', 'xenon.controllers.orders', 'xenon.controllers.materials', 'xenon.directives', 'xenon.factory', 'xenon.services', 'datatables', 'FBAngular', 'ngSails', 'ngSanitize', 'rt.select2' ]);
...
//	    });
	...
	if (message.verb === "reload") {
	 ...
	 var getType = {};
	 if($rootScope[message.verb] && getType.toString.call($rootScope[message.verb]) === '[object Function]'){
	     $rootScope[message.verb](message.data);
	 }
...
//	}
//	if(message.verb=="orderUpdate"&&rootScope.lotStatusUpdate)
	//TODO QUANTITY
...
app.config(function($preloaded, $user, $stateProvider, $urlRouterProvider, $ocLazyLoadProvider, ASSETS) {
    ...
    .state('app.scan', {
	url : '/scan',
	templateUrl : appHelper.templatePath('scan/index'),
	resolve : {
    ...
    .state('app.history', {
	url : '/history',
	templateUrl : appHelper.templatePath('history/index'),
	resolve : {
	    jQueryValidate : function($ocLazyLoad) {
		return $ocLazyLoad.load([ ASSETS.forms.jQueryValidate, ]);
	    },
    ...
    .state('app.materials', {
	url : '/materials',
	templateUrl : appHelper.templatePath('materials/index'),
	resolve : {
	    jQueryValidate : function($ocLazyLoad) {
		return $ocLazyLoad.load([ ASSETS.forms.jQueryValidate, ]);
	    },
...
//    }).
	...
	resolve : {
	    deps : function($ocLazyLoad) {
		return $ocLazyLoad.load([ ASSETS.maps.vectorMaps, ASSETS.icons.meteocons ]);
	    }
	}
    }).
	...
	resolve : {
		...
		return $ocLazyLoad.load([ ASSETS.uikit.base, ASSETS.uikit.codemirror, ASSETS.uikit.marked, ]);
	    },
	...
	resolve : {
		...
		return $ocLazyLoad.load([ ASSETS.core.jQueryUI, ASSETS.extra.tocify, ASSETS.icons.elusive, ]);
	    },
	}
	...
	resolve : {
	    resources : function($ocLazyLoad) {
		return $ocLazyLoad.load([ ASSETS.core.jQueryUI, ASSETS.extra.tocify, ASSETS.icons.meteocons, ]);
	    },
	}
	...
	resolve : {
		...
		return $ocLazyLoad.load([ ASSETS.core.googleMapsLoader, ASSETS.icons.elusive, ]);
	    },
	}
	...
	resolve : {
		...
		return $ocLazyLoad.load([ ASSETS.core.jQueryUI, ASSETS.core.moment, ASSETS.extra.fullCalendar, ]);
	    },
	}
	...
	resolve : {
	    ...
	    dxCharts : function($ocLazyLoad) {
		return $ocLazyLoad.load([ ASSETS.charts.dxCharts, ASSETS.charts.dxVMWorld, ]);
	    },
	}
    }).
	...
	resolve : {
	    resources : function($ocLazyLoad) {
		return $ocLazyLoad.load([ ASSETS.forms.jQueryValidate, ASSETS.extra.toastr, ]);
	    },
	}
    }).state('login-light', {
	url : '/login-light',
