...
// Widgets Directives
  ...
  function () {
    ...
    return {
      ...
      link: function (scope, el, attrs) {
        ...
        sm.fullyEnterViewport(function () {
          ...
          }, $count = attrDefault($el, 'count', 'this') == 'this' ? $el : $el.find($el.data('count')), from = attrDefault($el, 'from', 0), to = attrDefault($el, 'to', 100), duration = attrDefault($el, 'duration', 2.5), delay = attrDefault($el, 'delay', 0), decimals = new String(to)
            .match(/\.([0-9]+)/) ? new String(to).match(/\.([0-9]+)$/)[1].length : 0, counter = new countUp($count.get(0), from, to, decimals, duration, opts);
    ...
    return {
      restrict: 'EAC',
      link: function (scope, el, attrs) {
        var $el = angular.element(el), $nav = $el.find('.xe-nav a'), $status_list = $el.find('.xe-body li'), index = $status_list.filter('.active').index(), auto_switch = attrDefault($el, 'auto-switch', 0), as_interval = 0;
...
// Forms Directives
  directive('tagsinput', function () {
    ...
    return {
      ...
      link: function (scope, el, attr) {
        ...
        var $this = angular.element(el), $label_1 = $('<span class="ui-label"></span>'), $label_2 = $label_1.clone(),
          ...
          step = attrDefault($this, 'step', 1), value = attrDefault($this, 'value', 5), min = attrDefault($this, 'min', 0), max = attrDefault($this, 'max', 100), min_val = attrDefault($this, 'min-val', 10), max_val = attrDefault($this, 'max-val', 90),
    ...
    return {
      ...
      link: function (scope, el, attr) {
        ...
        /*
         * $this.find('tr').unbind(); $this.find('tr').click(function(){
  ...
  }).directive('validate', function ($rootScope) {
    return {
      restrict: 'C',
      link: function (scope, el, attr) {
        ...
        opts['ignore'] = '';
        $rootScope.validator[el.attr('id')] = $this.validate(opts); // ties
        // the
        // validators
        // through
        // the
