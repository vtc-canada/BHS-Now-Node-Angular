angular
	...
	.directive('xeBreadcrumb', function() {
		...
		link : function(scope, el) {
			...
			var $as = $bc.find('li a'), collapsed_width = $as.width(), expanded_width = 0;
	...
	directive(
		...
		function() {
			...
			link : function(scope, el, attrs) {
			    var $el = angular.element(el), sm = scrollMonitor.create(el);
			    ...
			    sm
					...
					}, $count = attrDefault($el, 'count', 'this') == 'this' ? $el : $el.find($el.data('count')), from = attrDefault($el,
						'from', 0), to = attrDefault($el, 'to', 100), duration = attrDefault($el, 'duration', 2.5), delay = attrDefault(
						$el, 'delay', 0), decimals = new String(to).match(/\.([0-9]+)/) ? new String(to).match(/\.([0-9]+)$/)[1].length
						: 0, counter = new countUp($count.get(0), from, to, decimals, duration, opts);
	...
	.directive(
		...
		function() {
			...
			restrict : 'EAC',
			link : function(scope, el, attrs) {
			    var $el = angular.element(el), $nav = $el.find('.xe-nav a'), $status_list = $el.find('.xe-body li'), index = $status_list.filter(
				    '.active').index(), auto_switch = attrDefault($el, 'auto-switch', 0), as_interval = 0;
	...
	directive('tocify', function() {
		...
		link : function(scope, el, attr) {
		    ...
		    var $this = angular.element(el), watcher = scrollMonitor.create($this.get(0));
	...
	.directive('scrollable', function() {
		...
		link : function(scope, el, attr) {
		    ...
		    var $this = angular.element(el), max_height = parseInt(attrDefault($this, 'max-height', 200), 10);
	...
	.directive('wysihtml5', function() {
		...
		link : function(scope, el, attr) {
		    ...
		    var $this = angular.element(el), stylesheets = attrDefault($this, 'stylesheet-url', '')
	...
	.directive(
		...
		function() {
			...
			link : function(scope, el, attr) {
			    ...
			    var $this = angular.element(el), $label_1 = $('<span class="ui-label"></span>'), $label_2 = $label_1.clone(),

			    orientation = attrDefault($this, 'vertical', 0) != 0 ? 'vertical' : 'horizontal',

			    prefix = attrDefault($this, 'prefix', ''), postfix = attrDefault($this, 'postfix', ''),

			    fill = attrDefault($this, 'fill', ''), $fill = $(fill),

			    step = attrDefault($this, 'step', 1), value = attrDefault($this, 'value', 5), min = attrDefault($this, 'min', 0), max = attrDefault(
				    $this, 'max', 100), min_val = attrDefault($this, 'min-val', 10), max_val = attrDefault($this, 'max-val', 90),
				...
				$this.slider({
				    ...
				    step : step,
				    slide : function(e, ui) {
					var min_val = (prefix ? prefix : '') + ui.values[0] + (postfix ? postfix : ''), max_val = (prefix ? prefix : '')
						+ ui.values[1] + (postfix ? postfix : '');
	...
	.directive(
		...
		function() {
			...
			link : function(scope, el, attr) {
			    ...
			    var $this = $(el), $tabs = $this.find('> .tabs > li'), $progress = $this.find(".progress-indicator"), _index = $this.find(
				    '> ul > li.active').index();
	...
	.directive('colorpicker', function() {
		...
		link : function(scope, el, attr) {
		    ...
		    var $this = angular.element(el), opts = {}, $n = $this.next(), $p = $this.prev(),
	...
	.directive(
		...
		function($rootScope) {
			...
			link : function(scope, el, attr) {
			    ...
			    var $this = angular.element(el), opts = {
				rules : {},
				messages : {},
				errorElement : 'span',
			    ...
			    $fields
				    .each(function(j, el2) {
					var $field = $(el2), name = $field.attr('name'), validate = attrDefault($field, 'validate', '').toString(), _validate = validate
						.split(',');
		...
		link : function(scope, el, attr) {
		    ...
		    var $this = angular.element(el), mask = $this.data('mask').toString(), opts = {
		...
		link : function(scope, el, attr) {
			...
			if ($this.updateDatepicker) {
			    clearTimeout($this.updateDatepicker);
			}
			$this.updateDatepicker = setTimeout(function() {
		...
		function() {
			...
			link : function(scope, el, attr) {
			    ...
			    }, min_date = attrDefault($this, 'minDate', ''), max_date = attrDefault($this, 'maxDate', ''), start_date = attrDefault($this,
				    'startDate', ''), end_date = attrDefault($this, 'endDate', '');
		...
		function() {
			...
			restrict : 'AC',
			link : function(scope, el, attr) {
			    var $ig = angular.element(el), $dec = $ig.find('[data-type="decrement"]'), $inc = $ig.find('[data-type="increment"]'), $inp = $ig
				    .find('.form-control'),
	...
	directive('loginForm', function() {
		...
		link : function(scope, el) {
			...
			var $this = angular.element(el), $label = $this.find('label'), $input = $this.find('.form-control');
